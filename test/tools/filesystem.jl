@testset "Filesystem" verbose = false begin
    ...
    @testset "File Search" begin
        ...
        local_search_func = Anglerfish.local_search_func()
        @test local_search_func !== nothing
        ...
        local_search_tool = Anglerfish.TOOLS["local_search"]
        local_search_result = local_search_tool.handler(Dict("keywords" => ["file"], "directories" => [ro_dir], "only_files" => "true")).text |> JSON.parse
        @test any(endswith("test file 1.txt"), local_search_result["files"])

        # test handler with multiple keywords and specific directory provided
        local_search_result_multiple_keywords = local_search_tool.handler(Dict("keywords" => ["ÄÜÖ", "non_existent_file"], "directories" => [ro_dir], "only_files" => "True")).text |> JSON.parse
        @test any(endswith("Testdokument ÄÜÖ.md"), local_search_result_multiple_keywords["files"])
        @test !any(endswith("non_existent_file"), local_search_result_multiple_keywords["files"])

        # test handler with single keyword, specific directory, and file extension filter provided
        local_search_result_single_keyword_with_filter = local_search_tool.handler(Dict("keywords" => ["file"], "directories" => [ro_dir], "filter" => [".txt"], "only_files" => true)).text |> JSON.parse
        @test any(endswith("test file 1.txt"), local_search_result_single_keyword_with_filter["files"])
        @test all(endswith(".txt"), local_search_result_single_keyword_with_filter["files"])
    end
end